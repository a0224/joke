...
import cc.joke.R;
import cc.joke.util.FlowIndicator;
...
/**
 * A TitleFlowIndicator is a FlowIndicator which displays the title of left
...
public class TitleFlowIndicator extends TextView implements FlowIndicator
{
    ...
    // 裁剪区
    private static final float CLIP_PADDING = 5.0f;

    private static final int SELECTED_COLOR = 0xFFff7300;

    private static final boolean SELECTED_BOLD = false;

    private static final int TEXT_COLOR = 0x80ff7300;

    private static final int TEXT_SIZE = 18;

    private static final float FOOTER_LINE_HEIGHT = 2.0f;

    private static final int FOOTER_COLOR = 0xFFff7300;

    // private static final float FOOTER_TRIANGLE_HEIGHT = 4.0f;
    ...
    public TitleFlowIndicator(Context context, AttributeSet attrs)
    {
        ...
        // a.getDimension(R.styleable.TitleFlowIndicator_footerTriangleHeight,
     ...
     */
    private void initDraw(int textColor, float textSize, int selectedColor, boolean selectedBold, float selectedSize,
            float footerLineHeight, int footerColor)
    {
        paintText = new Paint();
    ...
    protected void onDraw(Canvas canvas)
    {
        ...
        if (isDraw)
        {
            ...
            // 没有内容的时候返回1
            // int count = viewPager!=null?viewPager.getChildCount():1;
            int count = (viewPager != null && viewPager.getAdapter() != null) ? viewPager.getAdapter().getCount() : 1;
            ...
            if (currentPosition > 0)
            {
                for (int iLoop = currentPosition - 1; iLoop >= 0; iLoop--)
                {
                    ...
                    if (bound.left < 0)
                    {
                        ...
                        // Except if there's an intersection with the right
                        // view
            ...
            if (currentPosition < count - 1)
            {
                for (int iLoop = currentPosition + 1; iLoop < count; iLoop++)
                {
                    ...
                    if (bound.right > getLeft() + getWidth())
                    {
                        ...
                        // Except if there's an intersection with the left
                        // view
    ...
    /**
     * Calculate the bounds for a view's title 计算标题视图的误差
     * 
     * @param index
     * @param paint
     * @return
     ...
     * (non-Javadoc)
     * @see
     * org.taptwo.android.widget.viewPager.ViewSwitchListener#onSwitched(android
     * .view.View, int)
     */
    @Override
     ...
     * (non-Javadoc)
     * @see
     * org.taptwo.android.widget.FlowIndicator#setviewPager(org.taptwo.android
     * .widget.viewPager)
     */
    ...
    /**
     * Set the title provider 设置标题提供者
     * 
     * @param provider
     */
    ...
    private int measureWidth(int measureSpec)
    {
        ...
        {
            throw new IllegalStateException("viewPager can only be used in EXACTLY mode.");
        }
        result = specSize;
        return result;
    }
