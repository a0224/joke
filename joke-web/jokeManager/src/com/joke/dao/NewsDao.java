...
public class NewsDao extends BaseDao {
	...
	public List<NewsPojo> list(final NewsPojo data) {
		return getHibernateTemplate().executeFind(new HibernateCallback() {
			public Object doInHibernate(Session session)
					throws HibernateException, SQLException {

				String sql = "select j.id,u.nick_name,j.title,j.descrip,j.img,j.memo,"
						+ "j.talk,j.motime,j.mouser,j.status from tb_news j "
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<JokePojo> getByCid(final NewsPojo data) {
		return getHibernateTemplate().executeFind(new HibernateCallback() {
			public Object doInHibernate(Session session)
					throws HibernateException, SQLException {
				StringBuffer sqlbuf = new StringBuffer();
				String sql = "select id from tb_news j where 1=1  ";
				sqlbuf.append(sql);
				sqlbuf.append(" and j.status <> -1 ");
