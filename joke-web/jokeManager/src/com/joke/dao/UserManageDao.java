...
public class UserManageDao extends BaseDao {
	...
	private static Log log = LogFactory.getLog(UserManageDao.class);
	...
	@SuppressWarnings("unchecked")
	public List<UserPojo> getUserByUserName(UserPojo user) {
		List<UserPojo> userList = getHibernateTemplate().find(
				"from UserPojo u where u.loginName=? and status <> "
						+ Constant.DELETE, user.getLoginName());
		return userList;
	}

	public UserPojo getById(UserPojo user) {
		return (UserPojo) getById(UserPojo.class, user.getId());
	}
